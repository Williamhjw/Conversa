...
const getCityCode = async (lat, lon) => {
    if (!AMAP_API_KEY) {
        throw new Error("天气服务未配置");
    }
...
const buildWeatherData = (lives, cityName) => {
    return {
...
const getWeather = async (lat, lon) => {
    const { cityCode, cityName } = await getCityCode(lat, lon);
    const lives = await fetchWeatherData(cityCode);
    return buildWeatherData(lives, cityName);
};
...
const buildSuggestionPrompt = (temperature, feelsLike, humidity, windSpeed, description, location) => {
    return `你是一位专业的生活顾问和气象专家。请根据以下详细的天气信息，为用户提供全面、实用、贴心的穿衣建议和出行提醒。
...
const parseSuggestionResponse = (text) => {
    ...
    return {
        ...
        travel: travelMatch ? travelMatch[1].trim() : ""
    };
};

const getSuggestion = async (weatherData) => {
    const { temperature, feelsLike, humidity, windSpeed, description, location } = weatherData;
    ...
    try {
        const prompt = buildSuggestionPrompt(temperature, feelsLike, humidity, windSpeed, description, location);
        const text = await callGLMApi(prompt);
        ...
        const { clothing, travel } = parseSuggestionResponse(text);

        return {
            clothing: clothing || generateDefaultClothingSuggestion(temperature, feelsLike, humidity, windSpeed, description),
            travel: travel || generateDefaultTravelSuggestion(temperature, feelsLike, humidity, windSpeed, description)
        };
    } catch (error) {
...
module.exports = {
    ...
    estimateVisibility,
    estimatePressure,
    generateDefaultClothingSuggestion,
    generateDefaultTravelSuggestion,
};
