...
const initProblems = async (req, res) => {
  try {
    const count = await LeetCodeProblem.countDocuments();
    if (count > 0) {
      return res.json({ message: "题目已初始化", count });
    }
    ...
    res.json({ message: "初始化成功", count: problems.length });
  } catch (error) {
...
const toggleSolved = async (req, res) => {
  try {
    ...
    if (existing) {
      ...
      res.json({ solved: progress.solved });
    }
  } catch (error) {
...
const getProgress = async (req, res) => {
  try {
    ...
    const easySolved = await LeetCodeProgress.countDocuments({
      userId,
      solved: true,
      problemId: { $in: (await LeetCodeProblem.find({ difficulty: "Easy" })).map((p) => p.problemId) },
    });
    const mediumSolved = await LeetCodeProgress.countDocuments({
      userId,
      solved: true,
      problemId: { $in: (await LeetCodeProblem.find({ difficulty: "Medium" })).map((p) => p.problemId) },
    });
    const hardSolved = await LeetCodeProgress.countDocuments({
      userId,
      solved: true,
      problemId: { $in: (await LeetCodeProblem.find({ difficulty: "Hard" })).map((p) => p.problemId) },
    });
